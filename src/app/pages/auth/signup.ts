      ...
      <div class="w-full max-w-md rounded-2xl border border-slate-200 bg-white p-8 shadow-sm">
        ...
        <div class="mb-8 flex items-center gap-3">
          <div class="flex h-11 w-11 items-center justify-center rounded-xl bg-indigo-600 text-white">
            <svg lucideBriefcaseBusiness class="h-6 w-6"></svg>
          </div>
          <span class="text-xl font-bold text-slate-900">ApplyTrack</span>
        </div>

        <h1 class="text-2xl font-bold text-slate-900">Create your account</h1>
        <p class="mt-2 text-sm text-slate-500">Start tracking your job hunt in one place.</p>
        ...
        <form [formGroup]="form" (ngSubmit)="onSubmit()" class="mt-8 space-y-5">
          <div>
            ...
            @if (form.controls.fullName.touched && form.controls.fullName.invalid) {
              <p class="mt-1 text-sm text-red-600">Enter your full name</p>
            }
          </div>
        ...
        <button
          ...
          class="flex w-full cursor-pointer items-center justify-center gap-3 rounded-lg border border-slate-300 px-5 py-2.5 font-medium text-slate-700 transition hover:bg-slate-50"
        >
